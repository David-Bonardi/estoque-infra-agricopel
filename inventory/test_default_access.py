...
class DefaultUserAccessTests(TestCase):
    def test_new_user_can_register_stock_without_admin_access(self):
        ...
        self.assertEqual(self.client.post('/cadastros/filiais/novo/', {'code': '01', 'name': 'Filial', 'active': 'on'}).status_code, 302)
        self.assertTrue(Branch.objects.filter(code='01').exists())
